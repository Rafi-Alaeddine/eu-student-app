// src/services/transitAggregatorService.js
// Transit Aggregator Service (free-first)
// - Uses free sources in this order:
//   1) transport.rest (v6) — quick JSON endpoints where available
...
class TransitAggregatorService {
  constructor() {
    ...
    // defaults for mock fallback
    this.defaultFallbackCenter = { lat: 48.1351, lon: 11.5820 }; // Munich
  }

  // Geocode a city using Nominatim (public). Respect Nominatim usage policy.
  ...
  // Public method: get stops normalized to the mobile app contract
  async getStopsForCity(city, mode = 'bus') {
    let center;
    try {
      center = await this.geocodeCity(city);
    } catch (e) {
      // geocode failed; fall back to default
      center = this.defaultFallbackCenter;
    }

    // 1) Try transport.rest (fast JSON)
    try {
    ...
    // 2) Try TransitLand stops discovery
    try {
    ...
    // 3) Fallback: generate mock stops near city center
    return await this._mockStops(city, center);
  }

  // Public method: get vehicles normalized to the mobile app contract
  async getVehiclesForCity(city, mode = 'bus') {
    ...
    // For Phase A: no GTFS-rt ingestion yet — fallback to mock vehicles
    try {
      // Placeholder for future: discover GTFS-rt feed via TransitLand and parse it
    } catch (e) {
      console.warn('TransitLand vehicles attempt failed:', e.message);
    }

    // Fallback to mock vehicles (positioned near discovered stops)
  ...
  async _getStopsFromTransportRest(center, mode) {
    ...
    const stops = resp.data.map((s) => {
      ...
      const lat = coords[1];
      return {
        id: s.id || s.properties?.id || `tr:${s.name || Math.random()}`,
        name: s.name || s.properties?.name || 'Unknown',
        lat: Number(lat),
        lon: Number(lon),
      };
  ...
  async _getStopsFromTransitLand(center, mode) {
    ...
    const resp = await axios.get(url, {
      timeout: 10000,
      headers: { 'User-Agent': 'denti-transport-backend/1.0' },
    });
  ...
  // Mock stops generator (used as fallback)
  async _mockStops(city, center = null) {
    if (!center) {
      center = await this.geocodeCity(city).catch(() => this.defaultFallbackCenter);
    }
  ...
  // Mock vehicles generator based on stops
  _mockVehiclesFromStops(city, mode, stops = [], center = null) {
    const vehicles = [];
    if (!stops || stops.length === 0) {
      return this._mockVehicles(city, mode);
    }
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      vehicles.push({
        ...
        route: `R${(i % 10) + 1}`,
        delay_seconds: Math.round((Math.random() - 0.5) * 120),
      });
    }
    return vehicles;
  }

  // Generic mock vehicles if nothing else available
